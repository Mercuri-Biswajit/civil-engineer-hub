...
const Input = forwardRef(({
  ...
  const inputClasses = [
    ...
    error 
      ? "border-red-400 bg-red-500/5 focus:border-red-500 shadow-[0_0_0_3px_rgba(239,68,68,0.08)]" 
      : "border-slate-200 focus:border-indigo-500 focus:shadow-[0_0_0_3px_rgba(99,102,241,0.08)]",
    leftIcon ? "pl-11" : "",
    rightIcon ? "pr-11" : "",
  ].filter(Boolean).join(' ');
  ...
  return (
    <div className={containerClasses}>
      ...
      {(error || helperText) && (
        <div className={`text-xs font-semibold mt-1 ${error ? 'text-red-500' : 'text-slate-400'}`}>
          {error || helperText}
        </div>
      )}
    </div>
